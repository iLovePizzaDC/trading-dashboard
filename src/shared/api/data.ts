...
function parseEquityCsv(text: string): EquityPoint[] {
...
export async function fetchSummary(version: string): Promise<Summary> {
	const res = await fetchOk(`/data/summary.json?v=${version}`);
	return res.json();
}

export async function fetchDeposits(version: string): Promise<Deposit[]> {
	const res = await fetchOk(`/data/deposits.json?v=${version}`);
	return res.json();
}

export async function fetchBotEquity(version: string): Promise<EquityPoint[]> {
	const res = await fetchOk(`/data/live_equity.csv?v=${version}`);
	return parseEquityCsv(await res.text());
}

export async function fetchSpyEquity(version: string): Promise<EquityPoint[]> {
	const res = await fetchOk(`/data/spy_equity.csv?v=${version}`);
	return parseEquityCsv(await res.text());
}

export async function fetchTrades(version: string): Promise<Trade[]> {
	const res = await fetchOk(`/data/live_trades.json?v=${version}`);
	return res.json();
}

export async function fetchOpenStops(version: string): Promise<OpenStops> {
	const res = await fetchOk(`/data/open_stops.json?v=${version}`);
	return res.json();
}

export async function fetchStopHistory(version: string): Promise<StopHistory> {
	const res = await fetchOk(`/data/stop_history.json?v=${version}`);
	return res.json();
}

export async function fetchDecisions(version: string): Promise<DecisionEntry[]> {
	const res = await fetchOk(`/data/decisions_log.json?v=${version}`);
	return res.json();
}

export async function fetchRegime(version: string): Promise<RegimeEntry[]> {
	const res = await fetchOk(`/data/regime_log.csv?v=${version}`);
...
export async function fetchLastRebalanceDate(version: string): Promise<string> {
	const res = await fetchOk(`/data/last_rebalance.txt?v=${version}`);
	return await res.text();
}

export async function fetchLastWeeklyReportDate(version: string): Promise<string> {
	const res = await fetchOk(`/data/last_weekly_report.txt?v=${version}`);
	return await res.text();
}

export async function fetchMarketStatus(version: string): Promise<MarketStatus> {
	const res = await fetchOk(`/data/market_status.json?v=${version}`);
